...
if gpus:
    try:
        ...
        print("[TF] memory_growth set failed:", e)
...
ROUND_TRIALS = int(os.getenv("ROUND_TRIALS", "5000"))

# Per-algo targets (safe defaults)
TARGET_TRIALS_NN  = int(os.getenv("TARGET_TRIALS_NN",  str(OPTUNA_TRIALS)))
TARGET_TRIALS_XGB = int(os.getenv("TARGET_TRIALS_XGB", str(OPTUNA_TRIALS)))
TARGET_TRIALS_CAT = int(os.getenv("TARGET_TRIALS_CAT", str(OPTUNA_TRIALS)))
TARGET_TRIALS_LGB = int(os.getenv("TARGET_TRIALS_LGB", str(OPTUNA_TRIALS)))
...
# NN progress control
NN_PROGRESS = int(os.getenv("NN_PROGRESS", "1")) 
NN_PROGRESS_EVERY_SEC = float(os.getenv("NN_PROGRESS_EVERY_SEC", "2.0"))
...
X_tree_all = X_clean[feat_cols].copy()    
...
Y = df[THICKNESS_COLS].ffill().fillna(0.0).values.astype(np.float32)
...
def lgb_model_factory(params, Xtr, ytr_j, Xva, yva_j):
    try:
        ...
        if _supports_fit_arg(LGBMRegressor, "early_stopping_rounds"):
            ...
            bst = lgb.train(
                low, dtrain, num_boost_round=num_boost, valid_sets=[dtrain, dval],
...
def cat_model_factory(params, Xtr, ytr_j, Xva, yva_j):
    try:
        cat_feat = [c for c in categorical_cols if isinstance(Xtr, pd.DataFrame) and c in Xtr.columns] if isinstance(Xtr, pd.DataFrame) else []
...
def make_keras_model(input_dim: int, hp: dict):
    ...
    for _ in range(int(hp.get("n_layers", 2))):
        model.add(
            layers.Dense(
                int(hp.get("units", 128)),
                activation=hp.get("activation", "relu"),
                kernel_regularizer=keras.regularizers.l2(float(hp.get("weight_decay", 0.0)))
            )
        )
...
class ThrottledTrainProgress(keras.callbacks.Callback):

    def __init__(self, every_sec: float = 2.0, prefix: str = "NN fit"):
        super().__init__()
    ...
    def on_train_begin(self, logs=None):
        ...
        self.epochs = int(self.params.get("epochs", 0) or 0)
        self.epoch = 0
        self.batch = 0
    ...
    def on_train_batch_end(self, batch, logs=None):
        self.batch = int(batch) + 1
        if not NN_PROGRESS:
            return
        now = time.time()
        if (now - self.last) >= self.every_sec:
            elapsed = now - self.t0
            spe = self.steps_per_epoch or 0
            if spe > 0 and self.epochs:
                done_steps = (self.epoch - 1) * spe + self.batch
                total_steps = self.epochs * spe
                msg = f"\r{self.prefix}: step {done_steps}/{total_steps} | epoch {self.epoch}/{self.epochs} | batch {self.batch}/{spe} | elapsed {elapsed:,.0f}s"
            else:
                msg = f"\r{self.prefix}: epoch {self.epoch} batch {self.batch} | elapsed {elapsed:,.0f}s"
...
def nn_model_factory(hp, Xtr, ytr_j, Xva, yva_j):
    ...
    Xtr = to_dense_np32(Xtr)
    Xva = to_dense_np32(Xva)
    ytr_j = np.asarray(ytr_j, dtype=np.float32).reshape(-1)
    yva_j = np.asarray(yva_j, dtype=np.float32).reshape(-1)
    ...
    callbacks = [
        keras.callbacks.EarlyStopping(
            monitor="val_loss",
            patience=max(5, int(EARLY_STOP // 10)),
            restore_best_weights=True,
            verbose=0
        )
    ]
    if NN_PROGRESS:
        callbacks.append(ThrottledTrainProgress(every_sec=NN_PROGRESS_EVERY_SEC, prefix="NN fit"))
    ...
    preds = model.predict(Xva, verbose=1).reshape(-1)

    del Xtr, Xva, ytr_j, yva_j
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    for j, tcol in enumerate(THICKNESS_COLS):
        ...
        try:
            if algo == "xgb":
                ...
                X_train = X_all.toarray() if hasattr(X_all, "toarray") else np.asarray(X_all)
                ...
                model.fit(X_train, y_full_j.astype(np.float32), epochs=hp["epochs"], batch_size=hp["batch_size"], callbacks=[cb], verbose=0)
                models[tcol] = model
                gc.collect()
                tf.keras.backend.clear_session()
            else:
...
def train_final_models_and_stack(best_trials_info: dict):
    ...
    try:
        src_df = X_tree_all if isinstance(X_tree_all, pd.DataFrame) else None
        cat_mappings = {}
        ...
        for j, tcol in enumerate(THICKNESS_COLS):
            ...
            try:
                ...
                if algo == "xgb":
                    ...
                    Xf = X_all.toarray() if hasattr(X_all, "toarray") else np.asarray(X_all)
                    Xf = np.asarray(Xf, dtype=np.float32)
                    model = make_keras_model(Xf.shape[1], hp)
                    model.fit(Xf, y_full_j.astype(np.float32), epochs=hp["epochs"], batch_size=hp["batch_size"], verbose=0)
                    gc.collect()
                    tf.keras.backend.clear_session()
