# -*- coding: utf-8 -*-
"""
Training script v1.2.61 (XGBoost per-point + optional CatBoost multi-output comparison)

Features:
- Existing per-point XGBoost pipeline with non-overlapping TimeSeries CV + gap
- Parallel XGBoost final training via joblib (one model per thickness point)
- Optional CatBoost multi-output training (MultiRMSE) run in parallel for comparison
- Saves artifacts: preprocessor.joblib, xgb_A..xgb_G.joblib, catboost_all_points.joblib (if used)
- Maintains DB writes (types/specs/feature_bounds/model_registry)
"""
...
# ---------------- Config (env-driven) ----------------
MODEL_VERSION = os.getenv("MODEL_VERSION", "v1.2.61")
...
CORR_DROP_THR      = float(os.getenv("CORR_DROP_THR", "1.0"))
...
# Objective
XGB_OBJECTIVE = os.getenv("XGB_OBJECTIVE", "reg:squarederror")
...
# CatBoost toggles (new)
USE_CATBOOST = int(os.getenv("USE_CATBOOST", "0"))
CATBOOST_LOSS = os.getenv("CATBOOST_LOSS", "MultiRMSE")
CATBOOST_EARLY_STOP = int(os.getenv("CATBOOST_EARLY_STOP", "100"))
CATBOOST_DEPTH = int(os.getenv("CATBOOST_DEPTH", "6"))
CATBOOST_LR = float(os.getenv("CATBOOST_LR", "0.05"))
...
# ---------- Feature selection and engineering (unchanged) ----------
...
def save_catboost_artifact(cb_model, model_dir: str):
    os.makedirs(model_dir, exist_ok=True)
    joblib.dump(cb_model, os.path.join(model_dir, "catboost_all_points.joblib"))
...
# ---------- Parallel XGB helper ----------
def _train_single_point(args):
    tcol, Xtr_t, ytr, Xva_t, yva, params = args
    model = XGBRegressor(**params)
    model = _xgb_fit_with_es(model, Xtr_t, ytr, Xva_t, yva, early_rounds=EARLY_STOP_ROUNDS)
    er = _evals_result_any(model)
    train_rmse = er.get("validation_0", {}).get("rmse", [])
    valid_rmse = er.get("validation_1", {}).get("rmse", [])
    best_it = int(np.argmin(valid_rmse)) if valid_rmse else None
    return (tcol, model, {"train": train_rmse, "valid": valid_rmse}, best_it)

# ---------- CV training (XGB only for now) ----------
def cv_train(df: pd.DataFrame, feature_cols: List[str], numeric_cols: List[str], categorical_cols: List[str], tuned: bool = False):
    ...
    for fold_idx, (tr_idx, va_idx, info) in enumerate(splits):
        ...
        # Optionally: CatBoost multi-output CV on same fold (if enabled)
        catboost_fold_metrics = None
        if USE_CATBOOST and HAS_CATBOOST:
            try:
                # Prepare y matrix (n_samples x 7)
                ytr_mat = tr_df[THICKNESS_COLS].values
                yva_mat = va_df[THICKNESS_COLS].values
                # CatBoost Pool (cat_features=None because we've already one-hot encoded later)
                cb_params = dict(iterations=3000, learning_rate=CATBOOST_LR, depth=CATBOOST_DEPTH,
                                 loss_function=CATBOOST_LOSS, random_seed=RANDOM_STATE, verbose=False)
                cb = CatBoostRegressor(**cb_params)
                # Fit on raw numeric/categorical via pre.transform -> but CatBoost can accept numpy X
                cb.fit(Xtr_t, ytr_mat, eval_set=(Xva_t, yva_mat), early_stopping_rounds=CATBOOST_EARLY_STOP)
                # predict returns shape (n_samples, n_targets)
                yhat_mat = cb.predict(Xva_t)
                # compute per-target RMSE/MAE
                catboost_fold_metrics = {}
                for i, tcol in enumerate(THICKNESS_COLS):
                    yh = yhat_mat[:, i]
                    ytrue = yva_mat[:, i]
                    mae = float(np.mean(np.abs(yh - ytrue)))
                    rmse = float(np.sqrt(np.mean((yh - ytrue)**2)))
                    catboost_fold_metrics[tcol] = {"MAE": mae, "RMSE": rmse, "n_val": int(len(ytrue))}
            except Exception as e:
                print(f"CatBoost CV fold train failed: {e}")
                catboost_fold_metrics = None
        ...
        fold_details.append({
            ...
            "catboost_fold_metrics": catboost_fold_metrics
        })
...
# ---------- Final training (XGB parallel + optional CatBoost multi-output) ----------
def train_final_models(df: pd.DataFrame, feature_cols: List[str], numeric_cols: List[str], categorical_cols: List[str]):
    ...
    for tcol in THICKNESS_COLS:
        ...
        if tcol in REG_OVERRIDES: params.update(REG_OVERRIDES[tcol])
        if mono is not None: params["monotone_constraints"] = mono
        tasks.append((tcol, Xtr_t, ytr, Xva_t, yva, params))

    results = Parallel(n_jobs=min(len(tasks), os.cpu_count()), backend='threading')(
        delayed(_train_single_point)(task) for task in tasks
    )

    xgb_models, curves, best_iters = {}, {}, {}
    for tcol, model, curve_dict, best_it in results:
        xgb_models[tcol] = model
        curves[tcol] = curve_dict
        best_iters[tcol] = best_it

    # Optional CatBoost multi-output final fit
    cb_model = None
    cb_metrics = None
    if USE_CATBOOST and HAS_CATBOOST:
        try:
            # Train CatBoost on whole train_df (multi-output)
            ytr_mat = train_df[THICKNESS_COLS].values
            yva_mat = valid_df[THICKNESS_COLS].values
            cb_params = dict(iterations=3000, learning_rate=CATBOOST_LR, depth=CATBOOST_DEPTH,
                             loss_function=CATBOOST_LOSS, random_seed=RANDOM_STATE, verbose=False)
            cb_model = CatBoostRegressor(**cb_params)
            cb_model.fit(Xtr_t, ytr_mat, eval_set=(Xva_t, yva_mat), early_stopping_rounds=CATBOOST_EARLY_STOP)
            yhat_mat = cb_model.predict(Xva_t)
            cb_metrics = {}
            for i, tcol in enumerate(THICKNESS_COLS):
                yh = yhat_mat[:, i]
                ytrue = yva_mat[:, i]
                mae = float(np.mean(np.abs(yh - ytrue)))
                rmse = float(np.sqrt(np.mean((yh - ytrue)**2)))
                cb_metrics[tcol] = {"MAE": mae, "RMSE": rmse, "n_val": int(len(ytrue))}
        except Exception as e:
            print(f"CatBoost final training failed: {e}")
            cb_model = None
            cb_metrics = None
    elif USE_CATBOOST and not HAS_CATBOOST:
        print("USE_CATBOOST=1 but catboost Python package not installed. Skipping CatBoost.")

    return pre, xgb_models, cb_model, curves, best_iters, cb_metrics
...
def main():
    ...
    if USE_CV:
        ...
        point_metrics, wnrms_info, cv_curves, fold_details, baseline_summary = cv_train(
            df, feat_cols, numeric_cols, categorical_cols, tuned=bool(TUNE)
        )
    ...
    pre, xgb_models, cb_model, curves, best_iters, cb_metrics = train_final_models(df, feat_cols, numeric_cols, categorical_cols)
    ...
    _plot_feature_importance(xgb_models, pre, outdir, topn=20)
    ...
    # Save artifacts
    save_artifacts(pre, xgb_models, feat_cols, numeric_cols, categorical_cols, MODEL_DIR)
    if cb_model is not None:
        save_catboost_artifact(cb_model, MODEL_DIR)
    ...
    report = {
        ...
        "tuning": {"used": bool(TUNE)},
        "catboost": {
            "used": bool(USE_CATBOOST and HAS_CATBOOST),
            "metrics": cb_metrics if cb_metrics is not None else None,
            "note": None if (USE_CATBOOST and HAS_CATBOOST) else ("CatBoost not installed" if USE_CATBOOST else "CatBoost disabled")
        },
        "notes": {
            "feature_pruning": {
